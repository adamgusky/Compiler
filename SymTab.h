...
Size is the current number of lists in the separate
chaining hash table.
Contents is an array of lists (i.e. so points to the zeroth element in the array)
If current is not NULL, it points to the current (name, attribute) pair in the
symbol table.
...
SymTab * createSymTab(int size);
  /*
  PRE: size >= 0
  Size is an estimate of the number of items that will be stored in the
  symbol table.
  Return a pointer to a new symbol table.
  */

// In the following function, assume a precondition that table references a
...
with a NULL attribute, set current to reference the new (name, attribute)
pair and return 1. Otherwise do not change current and return 0.
*/

int findName(SymTab *table, char *name);
/* if name is in the symbol table, set current to reference the (name, attribute) pair
and return 1. Otherwise do not change current and return 0.
*/

int hasCurrent(SymTab *table);
// if current references a (name, attribute) pair, return 1. Otherwise return 0

void setCurrentAttr(SymTab *table, void * attr);
// PRE: hashcurrent() == 1
// change the attribute value of the current (name, attribute) pair to attribute

void * getCurrentAttr(SymTab *table);
// PRE: hasCurrent() == 1
// return the attribute in the current (name, attribute) pairs

char * getCurrentName(SymTab *table);
// PRE: hasCurrent() == 1
// return the name in the current (name, attribute) pairs
...
/*
  if all (name, attribute) pairs have been visited since the last call to startIterator,
  return 0.
